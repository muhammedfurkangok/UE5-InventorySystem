...
#include "UserInterface/Inventory/AmoundWidget.h"
...
{
	if (OriginatingItemSlot->InternalItemReference->ItemNumericData.bIsStackable &&
...
void UInventorySubmenu::DropButtonClicked()
{
	if (PlayerCharacter)
	{
		UE_LOG(LogTemp, Error, L"%s: Calling DropItem with %s", *FString(__FUNCTION__),
		       *OriginatingItemSlot->InternalItemReference->ItemTextData.ItemName.ToString());
		PlayerCharacter->DropItem(OriginatingItemSlot->InternalItemReference, OriginatingItemSlot->InternalItemReference->Quantity);
	}
	else
	{
